...
import { dispatchErrorToasts, dispatchSuccessToasts } from "../utils/dispatchers/toast";
  ...
  async (action) => {
    ...
    try {
      ...
      if (onSuccess) {
        ...
        dispatchSuccessToasts({ dispatch, payload: response, onSuccess });
      }
    } catch (error: any) {
      dispatchErrorToasts({ dispatch, payload: error, onError });
      const errorPayload = {
        error: error?.message,
